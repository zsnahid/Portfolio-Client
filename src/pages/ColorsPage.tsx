...
export default function ColorsPage() {
  ...
  const primaryColors = [
    { name: 'Forest Deep', value: '#232A2E', description: 'The deepest forest background', variable: '--color-forest-deep' },
    { name: 'Pine Dark', value: '#2D353B', description: 'Secondary dark backgrounds', variable: '--color-pine-dark' },
    { name: 'Forest Mid', value: '#343F44', description: 'Medium tone for cards and sections', variable: '--color-forest-mid' },
    { name: 'Sage', value: '#3D484D', description: 'Lighter neutral for interactive elements', variable: '--color-sage' },
  ];

  const accentColors = [
    { name: 'Snow', value: '#D3C6AA', description: 'Primary text color, warm cream', variable: '--color-snow' },
    { name: 'Mist', value: '#A7C080', description: 'Secondary text, soft sage green', variable: '--color-mist' },
    { name: 'Spring', value: '#83C092', description: 'Green accent for links and highlights', variable: '--color-spring' },
    { name: 'Bloom', value: '#E67E80', description: 'Coral accent for special elements', variable: '--color-bloom' },
  ];

  const neutralColors = [
    { name: 'Stone', value: '#4F585E', description: 'Dark gray for borders and dividers', variable: '--color-stone' },
    { name: 'Bark', value: '#859289', description: 'Medium gray for secondary text', variable: '--color-bark' },
    { name: 'Moss', value: '#9DA9A0', description: 'Light gray for subtle text', variable: '--color-moss' },
    { name: 'Fog', value: '#C5CDD0', description: 'Very light gray for backgrounds', variable: '--color-fog' },
  ];

  const functionalColors = [
    { name: 'Success', value: '#A7C080', description: 'Forest green for success states', variable: '--color-success' },
    { name: 'Warning', value: '#DBBC7F', description: 'Golden amber for warnings', variable: '--color-warning' },
    { name: 'Error', value: '#E67E80', description: 'Coral red for errors', variable: '--color-error' },
  ];
  ...
  const ColorCard = ({ color, showVariable = false }: { color: Color, showVariable?: boolean }) => (
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h3 className="font-title font-semibold text-snow group-hover:text-spring transition-colors">{color.name}</h3>
            <button
              onClick={() => copyToClipboard(color.name, color.value)}
          ...
          <p className="text-sm text-mist leading-relaxed">{color.description}</p>
          <div className="space-y-2">
            <Badge 
              variant="secondary" 
            ...
            {showVariable && (
              <Badge 
                ...
                onClick={() => copyToClipboard(color.name + '-css', color.variable)}
              >
                {color.variable}
              </Badge>
            )}
  ...
  const GradientCard = ({ gradient }: { gradient: Gradient }) => (
    <Card className="p-4 bg-pine-dark border-forest-mid hover:border-spring transition-all duration-200 group">
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h3 className="font-title font-semibold text-snow">{gradient.name}</h3>
            <button
              onClick={() => copyToClipboard(gradient.name, gradient.value)}
          ...
          <div className="space-y-1">
            <Badge variant="secondary" className="bg-forest-mid text-snow border-stone font-mono text-xs">
              {gradient.css}
            </Badge>
          </div>
        </div>
    ...
    <div className="space-y-8">
      ...
      <Card className="p-6 bg-pine-dark border-forest-mid">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <h3 className="font-title text-lg font-medium text-spring mb-2">Nature-Inspired</h3>
            <p className="text-mist text-sm">
              This palette draws inspiration from the tranquility and vitality of a lush forest, 
              evoking the serene beauty of evergreen landscapes while maintaining modern web accessibility standards.
            </p>
          </div>
          <div>
            <h3 className="font-title text-lg font-medium text-spring mb-2">Warm & Accessible</h3>
            <p className="text-mist text-sm">
              All color combinations meet WCAG AA standards for contrast, ensuring excellent readability 
              while providing a warm, inviting atmosphere that feels both professional and approachable.
            </p>
          </div>
        </div>
      </Card>
      ...
      <div className="space-y-6">
        <div>
          <h2 className="font-title text-2xl font-semibold text-snow mb-2">Functional Colors</h2>
          <p className="text-mist">Status and feedback colors with nature theming</p>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* Active States */}
          <Card className="p-6 bg-pine-dark border-forest-mid">
            <h3 className="font-title text-lg font-semibold text-snow mb-4">Active Elements</h3>
            <div className="space-y-4">
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 rounded bg-gradient-to-r from-spring to-mist"></div>
                <span className="text-mist text-sm">Spring → Mist gradient</span>
              </div>
              <div className="p-3 bg-gradient-to-r from-spring to-mist text-forest-deep rounded-lg nav-glow">
                <span className="font-medium">Active navigation item</span>
              </div>
            </div>
          </Card>
          ...
          {/* Hover States */}
          <Card className="p-6 bg-pine-dark border-forest-mid">
            <h3 className="font-title text-lg font-semibold text-snow mb-4">Hover States</h3>
            <div className="space-y-4">
              <div className="flex items-center space-x-3">
                <div className="w-4 h-4 rounded" style={{backgroundColor: '#3D484D'}}></div>
                <div className="w-4 h-4 rounded" style={{backgroundColor: '#83C092'}}></div>
          ...
          {/* Text Hierarchies */}
          <Card className="p-6 bg-pine-dark border-forest-mid">
            <h3 className="font-title text-lg font-semibold text-snow mb-4">Text Hierarchy</h3>
            <div className="space-y-3">
              <div className="flex items-center space-x-3">
                <div className="w-4 h-4 rounded" style={{backgroundColor: '#D3C6AA'}}></div>
                <span className="text-xs text-mist">Primary (Snow)</span>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-4 h-4 rounded" style={{backgroundColor: '#A7C080'}}></div>
                <span className="text-xs text-mist">Secondary (Mist)</span>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-4 h-4 rounded" style={{backgroundColor: '#9DA9A0'}}></div>
      ...
      <div className="space-y-6">
        <div>
          <h2 className="font-title text-2xl font-semibold text-snow mb-2">Live Preview</h2>
          <p className="text-mist">Interactive examples showing the color palette in action</p>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Sidebar Preview */}
          <Card className="p-6 bg-pine-dark border-forest-mid">
            <h3 className="font-title text-lg font-semibold text-snow mb-4">Sidebar Navigation</h3>
          ...
          {/* Card Preview */}
          <Card className="p-6 bg-pine-dark border-forest-mid">
            <h3 className="font-title text-lg font-semibold text-snow mb-4">Content Cards</h3>
            <div className="space-y-3">
              <div className="p-4 bg-forest-mid border border-stone rounded-lg hover:border-spring transition-colors cursor-pointer">
                <h4 className="font-title text-snow font-medium mb-2">Card Title</h4>
                <p className="text-mist text-sm mb-3">This is how content cards look with our color palette.</p>
                <div className="flex space-x-2">
                  <Badge className="bg-spring text-forest-deep">Primary Tag</Badge>
                  <Badge variant="outline" className="border-mist text-mist">Secondary Tag</Badge>
                </div>
              </div>
            </div>
          </Card>
      ...
      <div className="space-y-6">
        <div>
          <h2 className="font-title text-2xl font-semibold text-snow mb-2">Usage Examples</h2>
          <p className="text-mist">See how these colors work together in practice</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Button Examples */}
          <Card className="p-6 bg-pine-dark border-forest-mid">
          ...
          {/* Text Examples */}
          <Card className="p-6 bg-pine-dark border-forest-mid">
            <h3 className="text-lg font-semibold text-snow mb-4">Typography</h3>
            <div className="space-y-2">
              <p className="text-snow font-semibold">Primary text (Snow)</p>
              <p className="text-mist">Secondary text (Mist)</p>
              <p className="text-moss text-sm">Subtle text (Moss)</p>
              <a href="#" className="text-spring hover:text-bloom transition-colors">Link text (Spring → Bloom)</a>
            </div>
          </Card>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-semibold text-snow mb-2">Accessibility</h2>
          <p className="text-mist">All color combinations meet WCAG accessibility standards</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="p-6 bg-pine-dark border-forest-mid">
            <h3 className="text-lg font-semibold text-snow mb-4">Contrast Ratios</h3>
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center p-3 bg-spring rounded">
                <span className="text-forest-deep">Forest Deep on Spring</span>
                <Badge className="bg-spring text-forest-deep">AAA</Badge>
              </div>
            </div>
          </Card>
          ...
          <Card className="p-6 bg-pine-dark border-forest-mid">
            <h3 className="text-lg font-semibold text-snow mb-4">Design Principles</h3>
            <div className="space-y-3 text-sm">
              <div className="flex items-start space-x-3">
                <div className="w-2 h-2 bg-spring rounded-full mt-1.5"></div>
                <span className="text-mist">High contrast between text and backgrounds</span>
              </div>
              <div className="flex items-start space-x-3">
                <div className="w-2 h-2 bg-spring rounded-full mt-1.5"></div>
                <span className="text-mist">Color is never the only way to convey information</span>
              </div>
              <div className="flex items-start space-x-3">
                <div className="w-2 h-2 bg-spring rounded-full mt-1.5"></div>
                <span className="text-mist">Interactive elements have clear focus states</span>
              </div>
              <div className="flex items-start space-x-3">
                <div className="w-2 h-2 bg-spring rounded-full mt-1.5"></div>
                <span className="text-mist">Warm tones reduce eye strain</span>
              </div>
            </div>
          </Card>
        </div>
      ...
      <Card className="p-6 bg-pine-dark border-forest-mid">
        ...
        <div className="space-y-4">
          <div>
            <h3 className="text-lg font-medium text-spring mb-2">CSS Custom Properties</h3>
            <p className="text-mist text-sm mb-2">
              All colors are available as CSS custom properties in <code className="bg-forest-mid px-1 py-0.5 rounded text-snow text-xs">src/index.css</code>:
            </p>
            <div className="bg-forest-deep p-4 rounded-lg">
              <code className="text-sm text-mist font-mono">
                :root {'{'}
                <br />
          ...
          <div>
            <h3 className="text-lg font-medium text-spring mb-2">Tailwind Classes</h3>
            <p className="text-mist text-sm mb-2">
              Extended Tailwind configuration provides utility classes:
            </p>
            <div className="bg-forest-deep p-4 rounded-lg">
              <code className="text-sm text-mist font-mono">
                {'<div className="bg-forest-deep text-snow">'}
                <br />
                &nbsp;&nbsp;{'<h1 className="text-spring">Everforest Portfolio</h1>'}
                <br />
                {'</div>'}
              </code>
            </div>
